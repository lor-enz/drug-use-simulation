...
class AgentFactory():
    ...
    def create_random_agent(self):
        ...
        using = True if (random.random() <= settings.set.init_population_percentage_using) else False
        addicted = True if (
                    random.random() <= settings.set.init_population_percentage_using * settings.set.init_users_percentage_addicted) else False
